...
import {
    createSnapshotForEditor,
    listSnapshotsForDocument,
    loadSnapshotForDocument,
    restoreSnapshotIntoEditor,
} from "./core/snapshotService";
import { SnapshotMeta } from "./core/snapshotTypes";
import { createTimelinePanel } from "./ui/timelinePanel"
import { TimelineProvider } from "./ui/timelineView";
import { deleteSnapshotFile, deleteSnapshotFolder, listSnapshots as listSnapshotsFromStore } from "./core/snapshotStore";
...
vscode.commands.registerCommand("snapshotter.deleteSnapshot", async (node) => {
    ...
    if (!workspace || !node.snapshot) return;

    const relPath = path.relative(workspace.uri.fsPath, node.filePath);

    deleteSnapshotFile(workspace, relPath, node.snapshot.id);
...
vscode.commands.registerCommand("snapshotter.deleteFileSnapshots", async (node) => {
    ...
    deleteSnapshotFolder(workspace, relPath);
...
// context.subscriptions.push(openSplitTimelineCmd);
    ...
    const createSnapshotCmd = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            const snap = await createSnapshotForEditor(
                editor,
                hasSel ? "snippet" : "file",
                label || undefined
            );
            ...
            if (snap) {
                vscode.window.showInformationMessage(
                    `Snapshot created at ${new Date(snap.meta.createdAt).toLocaleTimeString()}`
                );
            }
        }
    );
    ...
    const listSnapshotsCmd = vscode.commands.registerCommand(
        "snapshotter.listSnapshots",
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return vscode.window.showErrorMessage("No active editor.");
            ...
            const action = await vscode.window.showQuickPick(
                ["Restore into editor", "Open diff"],
                { placeHolder: "Choose action" }
            );
            if (!action) return;

            const snap = loadSnapshotForDocument(editor.document, selected.meta.id);
            if (!snap) return vscode.window.showErrorMessage("Snapshot load failed.");
    ...
    // Timeline panel
    const openTimelineCmd = vscode.commands.registerCommand(
        "snapshotter.openTimeline",
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return vscode.window.showErrorMessage("No active editor.");
            ...
            const panel = createTimelinePanel(
                ...
                async (message) => {
                    ...
                    const snap = loadSnapshotForDocument(editor.document, message.id);
                    if (!snap) return vscode.window.showErrorMessage("Snapshot not found.");
...
const createSnippetSnapshotCmd = vscode.commands.registerCommand(
    "snapshotter.createSnippetSnapshot",
    async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        if (snap) {
            vscode.window.showInformationMessage(
                `Snippet snapshot saved (${snap.meta.label})`
            );
        }
    }
);
...
context.subscriptions.push(createSnippetSnapshotCmd);
    ...
    const experimentCmd = vscode.commands.registerCommand(
        "snapshotter.experimentMode",
        async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return vscode.window.showErrorMessage("No active editor.");
            ...
            const metas = listSnapshotsForDocument(document);
            if (metas.length === 0) {
                return vscode.window.showInformationMessage("No snapshots for this file.");
            }
            ...
            const snap = loadSnapshotForDocument(document, chosen.meta.id);
            if (!snap) return vscode.window.showErrorMessage("Snapshot load failed.");
            ...
            const watcher = vscode.window.onDidCloseTerminal(async (t) => {
                if (t.name !== "Experiment Mode") return;
...
async function openDiffForSnapshot(
    ...
    await vscode.commands.executeCommand(
        ...
        leftUri,
        document.uri,
        `Snapshot (${meta.label || meta.id}) ↔ Current`
    );
}

export function deactivate() {}
function getTimelineWebviewHTML(webview: vscode.Webview, extensionPath: string, metas: SnapshotMeta[], fileName: string) {
...
export function registerSplitTimeline(context: vscode.ExtensionContext) {
  const openSplitTimelineCmd = vscode.commands.registerCommand(
    "snapshotter.openSplitTimeline3D",
    async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
      ...
      const metas = listSnapshotsFromStore(workspace, relPath);

      const panel = vscode.window.createWebviewPanel(
        "snapshotterTimeline3D",
        `Snapshot Timeline — ${path.basename(document.fileName)}`,
      ...
      panel.webview.onDidReceiveMessage(async (msg) => {
        try {
          ...
          if (msg.type === "restore") {
            const snap = loadSnapshotForDocument(document, msg.id);
            if (!snap) return vscode.window.showErrorMessage("Snapshot not found.");
            ...
            if (confirm === "Restore") {
              ...
              if (!editor2) return vscode.window.showErrorMessage("No active editor to restore into.");
              await restoreSnapshotIntoEditor(editor2, snap);
              vscode.window.showInformationMessage("Snapshot restored.");
            }
            return;
          }
          ...
          if (msg.type === "deleteSnapshot") {
            ...
            deleteSnapshotFile(workspace, rel, msg.id);
            vscode.window.showInformationMessage("Snapshot deleted.");
            // refresh list and send to webview
            const newList = listSnapshotsFromStore(workspace, rel);
          ...
          if (msg.type === "deleteFile") {
            ...
            if (!workspace) return;
            const rel = path.relative(workspace.uri.fsPath, document.uri.fsPath).replace(/\\/g, "/");
            deleteSnapshotFolder(workspace, rel);
          ...
          if (msg.type === "createSnippetFromSelection") {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return vscode.window.showErrorMessage("No active editor.");
            if (editor.selection.isEmpty) return vscode.window.showErrorMessage("No selection to snapshot.");
            const label = await vscode.window.showInputBox({ prompt: "Label for snippet snapshot", value: "Snippet" });
            const snap = await createSnapshotForEditor(editor, "snippet", label || undefined);
            if (snap) {
